    ...
    <div className="w-80 h-full bg-zinc-900 border-l border-zinc-800 flex flex-col z-20 shadow-[-10px_0_20px_-10px_rgba(0,0,0,0.5)] overflow-hidden">
      ...
      <div className="h-[60%] flex flex-col overflow-hidden">
        <div className="px-4 border-b border-zinc-800 flex items-end pb-2.5 bg-zinc-900/50 sticky top-0 backdrop-blur-md h-14">
          <h2 className="text-sm font-bold text-zinc-100">Combat Log</h2>
        </div>
      
      <div className="flex-1 overflow-y-auto px-4 pt-4 pb-2 space-y-1 text-sm font-mono tracking-tight leading-relaxed">
        {logs.length === 0 ? (
          <div className="p-2 rounded bg-zinc-950/30 border border-zinc-800/30 shadow-sm text-zinc-600 italic">
            Waiting for battle...
          </div>
        ) : null}
        ...
        {logs.map((log) => (
          <div 
            key={log.id} 
            className={clsx(
              "px-2 py-1.5 rounded bg-zinc-950/30 border border-zinc-800/30 shadow-sm transition-all",
              log.type === 'player-attack' && 'text-blue-500 border-blue-900/30',
              log.type === 'enemy-attack' && 'text-red-500 border-red-900/30',
              log.type === 'ability' && 'text-cyan-500 font-semibold border-cyan-900/30 bg-cyan-500/10',
              log.type === 'system' && 'text-emerald-500 italic border-emerald-900/30',
            )}
          >
            {log.message}
          </div>
