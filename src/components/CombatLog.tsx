      ...
      <div className="flex-1 overflow-y-auto px-4 pt-4 pb-2 space-y-1 text-sm font-mono tracking-tight leading-relaxed">
        ...
        {logs.map((log) => (
          <div 
            key={log.id} 
            className={clsx(
              "px-2 py-1.5 rounded bg-zinc-900/30 border border-zinc-800/30 shadow-sm transition-all",
              log.type === 'player-attack' && 'text-blue-500 border-blue-900/30',
              log.type === 'enemy-attack' && 'text-red-500 border-red-900/30',
              log.type === 'ability' && 'text-sky-400 font-semibold border-cyan-900/30 bg-sky-400/10',
              log.type === 'system' && 'text-sky-400 italic border-emerald-900/30',
            )}
          >
            {log.message}
          </div>
