...
import { Ghost, User, Trees, Mountain, Flame, Gem, Flower2, Sprout } from 'lucide-react';
...
// Applied to the `.grid` element only (not the pan wrapper), so the existing
// drag/clamp/auto-follow camera math keeps working in flat screen pixels.
...
// Cheap deterministic hash so decorative tile props are stable across re-renders
// without needing to store them anywhere (same x,y always produces the same value).
...
// Experimental: purely decorative "standing" props scattered on empty floor tiles,
// just to preview the counter-rotation effect across more of the grid. Easy to
// delete or swap icons/odds — has no gameplay meaning.
const TILE_DECOR = [
  { icon: Flame, color: 'text-orange-500/70' },
  { icon: Gem, color: 'text-cyan-400/70' },
  { icon: Flower2, color: 'text-pink-400/70' },
  { icon: Sprout, color: 'text-emerald-500/70' },
];
const TILE_DECOR_CHANCE = 12; // out of 100
...
const PlayerSprite = memo(() => {
  ...
  return (
    ...
    >
      <div className="standing-sprite" style={{ transform: FLOOR_COUNTER_TILT, transformStyle: 'preserve-3d', transformOrigin: 'bottom center' }}>
...
const FloatingTextSprite = memo(({ id }: { id: string }) => {
  ...
  return (
    ...
    >
      <div className="standing-sprite absolute inset-0" style={{ transform: FLOOR_COUNTER_TILT, transformStyle: 'preserve-3d' }}>
        <div 
...
export function Grid() {
  ...
  const [zoom, setZoom] = useState(0.55); // camera zoom, independent of the UI/resolution scaleFactor — scroll to adjust
  ...
  // Scroll-wheel zoom. React's synthetic onWheel is passive, so preventDefault()
  // there won't stop page scroll — attach a real listener instead.
  useEffect(() => {
    const viewport = viewportRef.current;
    if (!viewport) return;
    const handleWheel = (e: WheelEvent) => {
      e.preventDefault();
      setZoom(prev => {
        const next = prev - Math.sign(e.deltaY) * 0.05;
        return Math.min(1.2, Math.max(0.25, next));
      });
    };
  ...
  for (let y = minY; y <= maxY; y++) {
    for (let x = minX; x <= maxX; x++) {
      ...
      // Experimental: scatter a standing decorative icon on some empty tiles
      ...
      cells.push(
        ...
        >
          {obstacle && (
            <div
              className="standing-sprite absolute inset-0 flex items-center justify-center z-10 pointer-events-none"
              style={{ transform: FLOOR_COUNTER_TILT, transformStyle: 'preserve-3d', transformOrigin: 'bottom center' }}
            >
              {obstacle.type === 'tree' && <Trees className="w-4/5 h-4/5 text-zinc-800/80" />}
              {obstacle.type === 'rock' && <Mountain className="w-4/5 h-4/5 text-zinc-800/80" />}
            </div>
          )}
          {TileDecor && (
            <div
              className="standing-sprite absolute inset-0 flex items-center justify-center z-[5] pointer-events-none opacity-90"
              style={{ transform: FLOOR_COUNTER_TILT, transformStyle: 'preserve-3d', transformOrigin: 'bottom center' }}
            >
              <TileDecor.icon className={cn("w-2/5 h-2/5", TileDecor.color)} strokeWidth={1.75} />
            </div>
          )}
        </div>
      );
  ...
  return (
    <div 
      ...
      style={{ perspective: FLOOR_PERSPECTIVE_PX, perspectiveOrigin: '50% 20%' }}
    ...
    >
      ...
      >
        <div 
          className="floor-plane relative grid gap-px bg-zinc-800 border border-zinc-700/50 rounded-xl overflow-hidden shadow-2xl" 
          style={{ 
