...
export function TownView() {
  ...
  return (
    <main className="flex-1 relative flex flex-col bg-zinc-900">
      {/* Town Content */}
        ...
        <div className="w-1/3 flex flex-col gap-4">
          ...
          <button 
            ...
            className={`flex items-center justify-between p-6 rounded-xl transition-colors text-left border ${
              vendorOpen 
                ? 'bg-sky-400/10 border-sky-400/30 hover:bg-sky-400/20' 
                : 'bg-zinc-900 hover:bg-zinc-800 border-zinc-800'
            }`}
          >
            <div>
              <div className={`flex items-center gap-2 font-bold text-lg mb-1 ${vendorOpen ? 'text-sky-400' : 'text-zinc-100'}`}>
                <Store className="w-5 h-5" />
                Vendor
              </div>
              <div className={vendorOpen ? 'text-orange-200/60' : 'text-zinc-400'}>
        ...
        <div className="flex-1 bg-zinc-900/50 border border-zinc-800/50 rounded-xl p-8 flex flex-col relative overflow-hidden">
          {vendorOpen ? (
            <div className="w-full h-full flex flex-col items-center justify-center text-center animate-in fade-in zoom-in-95 duration-200">
              <Store className="w-16 h-16 text-sky-400/20 mb-4" />
              <h2 className="text-2xl font-bold text-sky-400 mb-2">The Vendor is Open</h2>
              <p className="text-zinc-400 max-w-md mb-6">
                Click any item in your inventory to sell it for Gold. Better rarity and item level means more gold!
              </p>
              <div className="bg-zinc-900/50 border border-zinc-800 rounded-lg p-4 inline-flex items-center gap-3">
                <span className="text-sm text-zinc-400 uppercase tracking-widest">Your Wealth:</span>
                <span className="text-xl font-bold text-yellow-400 flex items-center gap-1.5"><Coins className="w-5 h-5"/> {gold}</span>
              </div>
            </div>
          ) : (
            <div className="w-full h-full flex flex-col items-center justify-center text-center animate-in fade-in zoom-in-95 duration-200">
              <h2 className="text-2xl font-bold text-zinc-100 mb-2">Welcome to Town</h2>
