...
import { getTileLightIntensity } from '../utils/lighting';
...
export function createBoundaryWallRenderer(): BoundaryWallRenderer {
  ...
  function update(
    ...
    const getBrightness = (x: number, y: number) => {
      ...
      if (!isVisible) return 0.0; // fully black in fog

      const intensity = getTileLightIntensity(clampX, clampY, playerPos, lootDrops, visibleTiles);
      return intensity;
    };
    ...
    const drawWallPlane = (x1: number, y1: number, x2: number, y2: number) => {
      ...
      let brightness = getBrightness(Math.min(x1, x2), Math.min(y1, y2));
    ...
    const drawVerticalLine = (x: number, y: number, b1: number, b2: number) => {
      let brightness = Math.max(b1, b2);
      if (x === w || y === h) {
        brightness *= 0.67;
      }
    ...
    for (let x = 0; x <= w; x++) {
      const bLeft = x > 0 ? getBrightness(x - 1, 0) : 0.1;
      const bRight = x < w ? getBrightness(x, 0) : 0.1;
      drawVerticalLine(x, 0, bLeft, bRight);
      
      const bLeftH = x > 0 ? getBrightness(x - 1, h) : 0.1;
      const bRightH = x < w ? getBrightness(x, h) : 0.1;
      drawVerticalLine(x, h, bLeftH, bRightH);
    }
    
    for (let y = 0; y <= h; y++) {
      if (y === 0 || y === h) continue; // Skip corners (already drawn by x loop)
      const bUp = y > 0 ? getBrightness(0, y - 1) : 0.1;
      const bDown = y < h ? getBrightness(0, y) : 0.1;
      drawVerticalLine(0, y, bUp, bDown);
      
      const bUpW = y > 0 ? getBrightness(w, y - 1) : 0.1;
      const bDownW = y < h ? getBrightness(w, y) : 0.1;
